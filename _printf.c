	...
	while (*format)
	{
		if (*format == '%' && *(format + 1))
		{
			if (*(format + 1) == 'c')
				...
				m += handle(va_arg(i, unsigned long int), 10, 0);
			else if (*(format + 1) == 'o')
				m += handle(va_arg(i, unsigned long int), 8, 0);
			else if (*(format + 1) == 'X')
				m += handle(va_arg(i, unsigned long int), 16, 0);
			else if (*(format + 1) == 'x')
				m += handle(va_arg(i, unsigned long int), 16, 1);
			else if (*(format + 1) == 'S')
